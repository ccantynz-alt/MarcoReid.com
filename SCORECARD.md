...
| 2 | AI form library (500+ pre-drafted forms per jurisdiction) | 🟡 | `/generate` front-door + template engine + 3 templates × 4 jurisdictions = 12 substantive drafts: NDA (mutual), Employment Agreement, and Will + Testamentary Trust (Wills Act / Succession Act / UPC compliant). Engine reads from `lib/templates/*` and the public intake automatically renders any registered (slug, jurisdiction) pair. Adding a new template is a single TS module. ~488 templates queued |
