...
| 2 | AI form library (500+ pre-drafted forms per jurisdiction) | 🟡 | `/generate` front-door + template engine + 2 templates × 4 jurisdictions = 8 substantive drafts: NDA (mutual) and Employment Agreement (statutory-statement-compliant per jurisdiction). Engine reads from `lib/templates/*` and the public intake automatically renders any registered (slug, jurisdiction) pair. Adding a new template is a single TS module. ~492 templates queued |
