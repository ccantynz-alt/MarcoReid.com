              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-navy-100 bg-navy-50">
                    ...
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-navy-400">LexisNexis</th>
                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-forest-600">Marco Reid</th>
                  </tr>
                </thead>
                <tbody>
                  {([
                    { feature: "Legal research", lexisnexis: true, marcoreid: true },
                    { feature: "Transparent pricing", lexisnexis: "Custom quotes only", marcoreid: "$99\u2013499/mo" },
                    { feature: "Voice dictation", lexisnexis: false, marcoreid: true },
                    { feature: "Practice management", lexisnexis: "Separate product (CounselLink)", marcoreid: true },
                    { feature: "Accounting research", lexisnexis: false, marcoreid: true },
                    { feature: "Cross-domain queries", lexisnexis: false, marcoreid: true },
                    { feature: "Citation verification", lexisnexis: "Shepard\u2019s ($)", marcoreid: "Free, included" },
                    { feature: "AI assistant", lexisnexis: "Lexis+ AI (extra cost)", marcoreid: "Included" },
                    { feature: "Court filing", lexisnexis: "CourtLink (separate subscription)", marcoreid: true },
                    { feature: "E-signatures", lexisnexis: false, marcoreid: true },
                  ] as const).map((row, i) => (
                    <tr key={row.feature} className={`border-b border-navy-50 ${i % 2 === 1 ? "bg-navy-50/50" : ""}`}>
                      ...
                      <td className="px-6 py-3 text-navy-400">
                        {typeof row.lexisnexis === "boolean"
                          ? row.lexisnexis ? <span className="text-forest-500">&#10003;</span> : <span className="text-red-400">&#10007;</span>
                          : row.lexisnexis}
                      </td>
