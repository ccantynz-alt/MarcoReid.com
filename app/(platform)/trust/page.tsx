...
export default async function TrustPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12 sm:px-8 lg:px-12">
      <h1 className="font-serif text-display text-navy-800 dark:text-white">Trust accounts</h1>
      ...
      <div className="mt-8 overflow-hidden rounded-2xl border border-navy-100 bg-white shadow-card dark:border-navy-700 dark:bg-navy-800">
        {accounts.length === 0 ? (
          ...
          <table className="w-full">
            ...
            <tbody>
              {accounts.map((a) => (
                <tr key={a.id} className="border-b border-navy-50 last:border-0 dark:border-navy-700 dark:hover:bg-navy-700/50">
                  <td className="px-6 py-4 text-sm font-medium text-navy-700 dark:text-navy-200">{a.client.name}</td>
                  <td className="px-6 py-4 text-sm text-navy-500 dark:text-navy-300">{a.currency}</td>
                  <td className="px-6 py-4 text-right text-sm font-semibold text-forest-600 dark:text-forest-400">
