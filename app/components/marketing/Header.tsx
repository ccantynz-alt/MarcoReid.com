    ...
    >
      ...
      <nav className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6 sm:px-8 lg:px-12">
        ...
        <div className="hidden items-center gap-8 md:flex">
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`min-h-touch inline-flex items-center text-sm font-medium transition-colors ${
                  scrolled
                    ? active
          ...
          <Link
            href="/contact"
            className={`ml-2 inline-flex min-h-touch items-center rounded-lg px-5 py-2.5 text-sm font-semibold shadow-sm transition-all duration-200 hover:shadow-md ${
              scrolled
        ...
        <button
          type="button"
          className={`inline-flex min-h-touch min-w-touch items-center justify-center transition-colors md:hidden ${
            scrolled
              ? "text-navy-500 hover:text-navy-800"
